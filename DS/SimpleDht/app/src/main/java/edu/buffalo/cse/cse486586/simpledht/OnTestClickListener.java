...
public class OnTestClickListener implements OnClickListener {
	...
	private class Task extends AsyncTask<Void, String, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			if (testQueryAll()) {
				publishProgress("QueryAll success\n");
			} else {
				publishProgress("QueryAll fail\n");
			}

			if (testDelete()) {

				publishProgress("DeleteAll success\n");
			} else {
				publishProgress("DeleteAll fail\n");
			}
			return null;
		}
		...
		private boolean testQueryAll(){
			Cursor mc;
			try {

				 mc = mContentResolver.query(mUri, null,"@",null,null);
